...
Declare @strXmachine AS NVarchar(MAX)
...
BEGIN
	SELECT @StrSql='INSERT INTO #MchId(Machineid)
		...
		SELECT @StrSql=@StrSql + @StrMPlantID + @StrMachine + @StrTPMMachines +@StrGroupID
		EXEC(@StrSql)
		...
		--select * from #ShiftwiseProductionForADay
		--return
END
----ER0291 Till here
--mod 4
...
---------------------------------by Mrudula for optimization------------------------------------
	print sysdatetime()
		...
		select @strsql = @strsql + @strmachine + @StrTPMMachines +@StrMPlantID++@strcomponentid+@stroperation+@StrGroupID
...
select @strsql = @strsql + ' SELECT distinct  S.MachineID as Mach,S.Component as Compn,S.Operation as Oprn,'
		...
		*/
		...
		Select @strsql=''
		select @strsql = 'insert into #ShiftProductionFromAutodataT2 (PlantID,MachineID,GroupID,Component,CompDescription,Operation,OpnDescription,CycleTime,LoadUnload,AvgLoadUnload,AvgCycleTime,OperationCount, '
		select @strsql = @strsql + 'Sdate,ShiftName,ShftStart,ShftEnd, 	MachineInterface,CompInterface,	OpnInterface,MachineDescription) '
		select @strsql = @strsql + ' SELECT  distinct PlantMachine.PlantID,machineinformation.machineid,PlantMachineGroups.GroupID, componentinformation.componentid,componentinformation.description, '
		...
		select @strsql = @strsql + ',machineinformation.interfaceid,componentinformation.interfaceid,componentoperationpricing.interfaceid,machineinformation.description,componentinformation.description,
		componentoperationpricing.description
...
ER0280 Commented Till Here **********************/		
		...
		BEGIN
			...
			Select @StrSql = @StrSql+ @strmachine + @StrTPMMachines + @strcomponentid + @stroperation
...
--FROM(Select T1.MachineID,T1.UstartShift,
----CAST((Sum(Operationcount))As Float)/CAST((Sum(IsNull(Operationcount,0))+(IsNull(RejCount,0))) AS Float)As QE
--cast((Sum(isnull(Operationcount,0))-Sum(IsNull(RejCount,0))) as float)/CAST((Sum(IsNull(Operationcount,0))) AS Float)As QE
--From #ShiftProductionFromAutodataT1 T1
--INNER JOIN #ShiftProductionFromAutodataT2  T2 ON
--		T1.MachineID = T2.MachineID and
--		T1.UDate=T2.Sdate and
...
FROM(Select T1.MachineID,T1.UstartShift,
--CAST((Sum(Operationcount))As Float)/CAST((Sum(IsNull(Operationcount,0))+(IsNull(RejCount,0))) AS Float)As QE
cast((Sum(isnull(Operationcount,0))-isnull(Sum(distinct RejCount),0)) as float)/CAST((Sum(IsNull(Operationcount,0))) AS Float)As QE
From #ShiftProductionFromAutodataT1 T1
INNER JOIN #ShiftProductionFromAutodataT2  T2 ON
		T1.MachineID = T2.MachineID and
		T1.UDate=T2.Sdate and
...
--		SELECT @strmachine1 = ' AND ( machine = ''' + @MachineID+ ''')'
		SELECT @strmachine1 = ' AND ( machine in (' + @MachineID +'))'	
		---mod 8
...
--			SELECT @strmachine1 = ' AND ( CO.machineID = ''' + @MachineID+ ''')'
			SELECT @strmachine1 = ' AND ( CO.machineID in (' + @MachineID+ '))'
			---mod 8
			END
			select @strcomponent1=''
			if isnull(@ComponentID, '') <> ''
...
--Select * from #ShiftProductionFromAutodataT2
	...
	BEGIN
		...
		Case when autodata.msttime< T.Shftstart then T.Shftstart else autodata.msttime end,   
		Case when autodata.ndtime> T.Shftend then T.Shftend else autodata.ndtime end,     
		T.Shftstart,T.Shftend,0,autodata.id,componentoperationpricing.Cycletime,componentoperationpricing.SubOperations FROM #T_autodata autodata  with(nolock)
		INNER JOIN  machineinformation ON autodata.mc = machineinformation.InterfaceID   
		INNER JOIN componentinformation ON autodata.comp = componentinformation.InterfaceID    
		...
		and componentoperationpricing.machineid=machineinformation.machineid   
		cross join(select distinct Shftstart,Shftend from #ShiftProductionFromAutodataT2) as T
		where
		((autodata.msttime >= T.Shftstart  AND autodata.ndtime <=T.Shftend)
		OR ( autodata.msttime < T.Shftstart  AND autodata.ndtime <= T.Shftend AND autodata.ndtime > T.Shftstart )
		OR ( autodata.msttime >= T.Shftstart   AND autodata.msttime <T.Shftend AND autodata.ndtime > T.Shftend )
		OR ( autodata.msttime < T.Shftstart  AND autodata.ndtime > T.Shftend) ) '
		select @strsql = @strsql 
		...
		--select * from #FinalRunTime where FromTm>='2021-11-09 06:00:00.000' and FromTm<='2021-11-09 14:00:00.000' and MachineID='SEW-110-04'
		--return
		...
		update #FinalRunTime set target=(t.tcount)
		from
		(
		...
		--select * from #FinalRunTime where FromTm>='2021-11-10 14:00:00.000' and ToTm<='2021-11-10 22:00:00.000' and MachineID='SEW-110-04'


		UPDATE #ShiftProductionFromAutodataT2 SET Targetcount=ISNULL(t1.tgt,0)
		FROM
		(SELECT MachineID,MachineInterface,component,Compinterface,Operation,fromtm,totm, SUM(ISNULL(Target,0)) AS tgt FROM #FinalRunTime
		GROUP BY MachineID,MachineInterface,Component,Compinterface,Operation,FromTm,ToTm
		)t1 INNER JOIN #ShiftProductionFromAutodataT2 ON #ShiftProductionFromAutodataT2.MachineID = t1.MachineID AND #ShiftProductionFromAutodataT2.MachineInterface = t1.machineinterface
		and #ShiftProductionFromAutodataT2.Component = t1.Component AND #ShiftProductionFromAutodataT2.CompInterface = t1.Compinterface
		 and #ShiftProductionFromAutodataT2.Operation = t1.Operation and #ShiftProductionFromAutodataT2.ShftStart=t1.FromTm and #ShiftProductionFromAutodataT2.ShftEnd=t1.ToTm
...
Begin
	select  #ShiftProductionFromAutodataT1.MachineID,
		...
		ROUND(#ShiftProductionFromAutodataT1.QualityEfficiency,2) as QualityEfficiency,
		...
		isnull(#ShiftProductionFromAutodataT2.CompDescription,'') as CompDescription,
		isnull(#ShiftProductionFromAutodataT2.Operation,'') as Operation,
		isnull(#ShiftProductionFromAutodataT2.OpnDescription,'') as OpnDescription,
		...
		isnull(isnull(#ShiftProductionFromAutodataT2.TargetCount,0)-isnull(#ShiftProductionFromAutodataT2.OperationCount,0),0) as Diff
...
If @param = 'Shift'
Begin
	select 	#ShiftProductionFromAutodataT1.GroupID,
	  #ShiftProductionFromAutodataT1.MachineID,
	  #ShiftProductionFromAutodataT1.MachineDescription MachineDescription,
		#ShiftProductionFromAutodataT1.OperatorId,#ShiftProductionFromAutodataT1.OperatorName,
		...
		#ShiftProductionFromAutodataT1.Ushift as shift,
		#ShiftProductionFromAutodataT2.TargetCount as Target,
		isnull(round((isnull(#ShiftProductionFromAutodataT2.TargetCount,0)-isnull(#ShiftProductionFromAutodataT2.OperationCount,0)),2),0) as Diff
	...
	from   #ShiftProductionFromAutodataT1 LEFT OUTER JOIN #ShiftProductionFromAutodataT2 ON
		...
		order by #ShiftProductionFromAutodataT1.GroupID, #ShiftProductionFromAutodataT1.UShift,#ShiftProductionFromAutodataT1.UDate,#ShiftProductionFromAutodataT1.MachineID
End
...
--dbo.f_formattime(sum(T1.frmtUtilisedTime), @timeformat) as frmtUtilisedTime
--from (	select 	#ShiftProductionFromAutodataT1.GroupID,
--	  #ShiftProductionFromAutodataT1.MachineID,
--	  #ShiftProductionFromAutodataT1.MachineDescription MachineDescription,
--		#ShiftProductionFromAutodataT1.OperatorId,#ShiftProductionFromAutodataT1.OperatorName,
--		ISNULL(ROUND(#ShiftProductionFromAutodataT1.ProductionEfficiency,2),0) as ProductionEfficiency,
...
--		ISNULL(ROUND(#ShiftProductionFromAutodataT1.OverallEfficiency,2),0) as OverallEfficiency,
--		isnull(#ShiftProductionFromAutodataT1.DownTime,0) as frmtDownTime,
--		isnull(#ShiftProductionFromAutodataT1.UtilisedTime,0) as frmtUtilisedTime, 
--		isnull(#ShiftProductionFromAutodataT2.Component,'') as Component,
--		isnull(#ShiftProductionFromAutodataT2.CompDescription,'') as CompDescription,
--		isnull(#ShiftProductionFromAutodataT2.Operation,'') as Operation,
--		isnull(#ShiftProductionFromAutodataT2.OpnDescription,'') as OpnDescription,
...
dbo.f_formattime(sum(T1.frmtDownTime), @timeformat) as TotalDownTime,
dbo.f_formattime(sum(T1.frmtUtilisedTime), @timeformat) as frmtUtilisedTime
from (	select distinct #ShiftProductionFromAutodataT1.GroupID,
	  #ShiftProductionFromAutodataT1.MachineID,
	  #ShiftProductionFromAutodataT1.MachineDescription MachineDescription,
		ISNULL(ROUND(#ShiftProductionFromAutodataT1.ProductionEfficiency,2),0) as ProductionEfficiency,
		ISNULL(ROUND(#ShiftProductionFromAutodataT1.AvailabilityEfficiency,2),0) as AvailabilityEfficiency,
...
Begin
	select  #ShiftProductionFromAutodataT1.MachineID,
		...
		max(#ShiftProductionFromAutodataT1.QualityEfficiency) as QE,
