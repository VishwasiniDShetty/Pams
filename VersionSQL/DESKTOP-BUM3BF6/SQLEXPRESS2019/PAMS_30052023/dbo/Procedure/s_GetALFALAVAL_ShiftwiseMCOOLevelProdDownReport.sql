...
-- [dbo].[s_GetALFALAVAL_ShiftwiseMCOOLevelProdDownReport]   '2022-05-09 06:55:00 AM','2022-05-10 06:55:00 AM','','','','Format-2'
**************************************************************************************************************/  
...
/* ------- Commented by raksha r on 18-may-22
...
Update #FinalTarget set ActualAvailableTime=T1.WorkingHours
FROM (
select * from ShiftWiseWorkingHours_ALFALAVAL
)T1 inner join #FinalTarget T2 on T1.ShiftName=T2.Shift
...
Begin
	...
	--update #FinalOutput set TotalDowntime=T.TotalDowntime,Prodhrs=T.Prodhrs,Reworkhrs=T.Reworkhrs from
	...
	--inner join #FinalOutput on #FinalOutput.pdate=T.PDate and #FinalOutput.Machineid=T.Machineid and #FinalOutput
	--.shift=T.shift
	...
	insert into #FinalOutput(PDate,Machineid,shift,TotalAvailabletime,Prodhrs,A,B,C,D,E,F,G,H,I,J,K,L,M,N,O,P,Q,others,TotalDowntime,
	ActualWorkinghrs,AE,PE,OE,ActualAvailabletime,TargetCount,ActualCount,Reworkhrs)
    select Convert(nvarchar(10),FromTm,120) as PDate,Machineid,shift,
	TotalAvailabletime,sum(CN) as Prodhrs, 
	sum(A) as A,sum(B) as B,sum(C) as C,sum(D) as D,
	sum(E) as E,sum(F) as F,sum(G) as G,sum(H) as H,sum(I) as I,
	sum(J) as J,sum(K) as K,sum(L) as L,sum(M) as M,
	sum(N) as N,sum(O) as O,sum(P) as P,sum(Q) as Q,
	sum(others) as others,sum(Downtime) as TotalDowntime,sum(Utilisedtime) as ActualWorkinghrs,
	Sum(AvailabilityEfficiency) as AE,Sum(ProductionEfficiency) as PE,Sum(OverAllEfficiency) as OE,
	ActualAvailabletime,
	SUM(TargetCount) as TargetCount,SUM(components) as ActualCount,sum(Reworkhrs) as Reworkhrs
	from #FinalTarget  
	where (Utilisedtime>0 or Downtime>0 or Components>0) 
	Group by Machineid,fromtm,shift,TotalAvailabletime,ActualAvailableTime
	Order by Machineid,FromTm

	UPDATE #FinalOutput SET operator = t2.opr 
	from(
	SELECT T.Machineid,Convert(nvarchar(10),T.FromTm,120) as PDate,T.shift,
		STUFF(ISNULL((SELECT ', ' + x.Operator
				FROM #FinalTarget x
				WHERE x.Machineid = t.Machineid and Convert(nvarchar(10),X.FromTm,120) = Convert(nvarchar(10),t.FromTm,120) and x.shift = t.shift
			    GROUP BY x.Operator
				FOR XML PATH (''), TYPE).value('.','VARCHAR(max)'), ''), 1, 2, '') [opr]      
	FROM #FinalTarget t)
	as t2 Inner Join #FinalOutput on t2.Machineid = #FinalOutput.Machineid and  
	t2.PDate = #FinalOutput.PDate and t2.shift = #FinalOutput.shift

	UPDATE #FinalOutput SET Component = t2.Component 
	from(
	SELECT T.Machineid,Convert(nvarchar(10),T.FromTm,120) as PDate,T.shift,
		STUFF(ISNULL((SELECT ', ' + x.Component
				FROM #FinalTarget x
				WHERE x.Machineid = t.Machineid and Convert(nvarchar(10),X.FromTm,120) = Convert(nvarchar(10),t.FromTm,120) and x.shift = t.shift
			    GROUP BY x.Component
				FOR XML PATH (''), TYPE).value('.','VARCHAR(max)'), ''), 1, 2, '') [Component]      
	FROM #FinalTarget t)
	as t2 Inner Join #FinalOutput on t2.Machineid = #FinalOutput.Machineid and  
	t2.PDate = #FinalOutput.PDate and t2.shift = #FinalOutput.shift

	UPDATE #FinalOutput SET Operation = t2.Operation 
	from(
	SELECT T.Machineid,Convert(nvarchar(10),T.FromTm,120) as PDate,T.shift,
		STUFF(ISNULL((SELECT ', ' + x.Operation
				FROM #FinalTarget x
				WHERE x.Machineid = t.Machineid and Convert(nvarchar(10),X.FromTm,120) = Convert(nvarchar(10),t.FromTm,120) and x.shift = t.shift
			    GROUP BY x.Operation
				FOR XML PATH (''), TYPE).value('.','VARCHAR(max)'), ''), 1, 2, '') [Operation]      
	FROM #FinalTarget t)
	as t2 Inner Join #FinalOutput on t2.Machineid = #FinalOutput.Machineid and  
	t2.PDate = #FinalOutput.PDate and t2.shift = #FinalOutput.shift

	select distinct PDate,Machineid,Operator,shift,Component,Operation,
	dbo.f_formattime(TotalAvailabletime,'hh') as TotalAvailabletime,dbo.f_formattime(Prodhrs,'hh') as Prodhrs, 
	dbo.f_formattime(A,'hh') as A,dbo.f_formattime(B,'hh') as B,dbo.f_formattime(C,'hh') as C,dbo.f_formattime(D,'hh') as D,
	dbo.f_formattime(E,'hh') as E,dbo.f_formattime(F,'hh') as F,dbo.f_formattime(G,'hh') as G,dbo.f_formattime(H,'hh') as H,
	dbo.f_formattime(I,'hh') as I,dbo.f_formattime(J,'hh') as J,dbo.f_formattime(K,'hh') as K,dbo.f_formattime(L,'hh') as L,
	dbo.f_formattime(M,'hh') as M,dbo.f_formattime(N,'hh') as N,dbo.f_formattime(O,'hh') as O,
	dbo.f_formattime(P,'hh') as P,dbo.f_formattime(Q,'hh') as Q, dbo.f_formattime(others,'hh') as others,
	dbo.f_formattime(TotalDowntime,'hh') as TotalDowntime,dbo.f_formattime(ActualWorkinghrs,'hh') as ActualWorkinghrs,
	Round(AE,2) as AE,Round(PE,2) as PE,Round(OE,2) as OE,
	--dbo.f_formattime(ActualAvailableTime,'hh') as ActualAvailabletime,
	ActualAvailabletime,
	ROUND(DeviationInActAvlTime,2) as DeviationInActAvlTime,
	ROUND(TargetCount,2) as TargetCount,ActualCount,dbo.f_formattime(Reworkhrs,'hh') as Reworkhrs
	from #FinalOutput  
	where (ActualWorkinghrs>0 or TotalDowntime>0 or ActualCount>0) 
	Order by Machineid,PDate,Shift
End
Else
Begin
	 select Convert(nvarchar(10),FromTm,120) as PDate,Machineid,Operator,shift,Component,Operation,
