...
Declare @strXMachine nvarchar(2000)
...
--END
	...
	BEGIN
		...
		ProductionEfficiency * 100 As PE,
		(QualityEfficiency * 100) As QE,
		--(ProductionEfficiency * AvailabilityEfficiency)*100 As OE,
		---mod 3
		--'OE' = CASE
				--WHEN isnull(Components,0) <> 0 then (ProductionEfficiency * AvailabilityEfficiency*100) * ((Components - isnull(rejection,0))/Components)
			--END,
		CASE WHEN isnull(Components,0) <> 0 THEN (ProductionEfficiency * AvailabilityEfficiency * QualityEfficiency)*100 when isnull(Components,0)=0 then 0 END As OE,
		---mod 3
