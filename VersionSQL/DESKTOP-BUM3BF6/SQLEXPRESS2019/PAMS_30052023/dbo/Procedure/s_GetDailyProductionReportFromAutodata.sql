...
Begin
	...
	Select @strXmachine = ' and ( EX.MachineID IN (' + @MachineID + '))'	
---mod 2
End
...
BEGIN
	...
	BEGIN
		...
		begin
			...
			SELECT @StrSql=@StrSql+' WHERE MachineInformation.MachineID in ( '+ @machineid+') AND PM.PlantID = ''' +@PlantID+''' '
...
BEGIN
	...
	select @strsql = 'insert into #DailyProductionFromAutodataT2 (Cdate,MachineID,GroupID,Component,CompDescription,Operation,OpnDescription,CycleTime,LoadUnload,AvgLoadUnload,AvgCycleTime,'
	select @strsql = @strsql + 'CountShift1,CountShift2,CountShift3,FromTm,ToTm)'
	select @strsql = @strsql + '( SELECT ''' +convert(nvarchar(20),@StartTime)+ ''', machineinformation.machineid,PlantMachineGroups.GroupID, componentinformation.componentid,componentinformation.description, '
	select @strsql = @strsql + ' componentoperationpricing.operationno,componentoperationpricing.description, '
	...
	select @strsql = @strsql + @StrMPlantid + @strmachine +@StrTPMMachines + @strcomponentid + @stroperation+@StrGroupID
	...
	select @strsql = @strsql + ' componentoperationpricing.cycletime, componentoperationpricing.machiningtime ,ComponentOperationPricing.SubOperations,componentinformation.description,componentoperationpricing.description)'
	print @strsql
	exec(@strsql)
...
--	select @strsql = @strsql + ' AVG(autodata.loadunload/autodata.partscount) * ISNULL(ComponentOperationPricing.SubOperations,1) AS AvgLoadUnload ' --DR0325 commented
	...
	select @strsql = @strsql + ' WHERE (autodata.ndtime > ''' + convert(nvarchar(20),@CurStart) + ''')'
	select @strsql = @strsql + ' AND (autodata.ndtime <= ''' + convert(nvarchar(20),@CurEndTime) + ''')'
	select @strsql = @strsql + @StrMPlantid + @strmachine +@StrTPMMachines+ @strcomponentid + @stroperation
...
( Select A.mc,SUM(A.Rejection_Qty) as RejQty,T1.Pdate from AutodataRejections A    
inner join Machineinformation M on A.mc=M.interfaceid    
inner join #DailyProductionFromAutodataT1 T1 on T1.MachineInterface=A.mc AND convert(nvarchar(10),(A.RejDate),120) in (convert(nvarchar(10),T1.Pdate,120)) 
inner join Rejectioncodeinformation R on A.Rejection_code=R.interfaceid    
...
Declare @strmachine3 nvarchar(max)
...
--	SELECT @strmachine3 = ' AND ( machine = ''' + @MachineID+ ''')'
	SELECT @strmachine3 = ' AND ( machine in (' + @MachineID+ '))'
	---mod 3
	END
if isnull(@ComponentID, '') <> ''
	BEGIN
...
--	SELECT @strmachine3 = ' AND ( CO.machineID = ''' + @MachineID+ ''')'
	SELECT @strmachine3 = ' AND ( CO.machineID in (' + @MachineID+ '))'
	---mod 3
	END
	select @strcomponent3=''
	if isnull(@ComponentID, '') <> ''
...
If (SELECT ValueInText From CockpitDefaults Where Parameter ='Ignore_Ptime_4m_PLD')='Y'
	BEGIN
		...
		BEGIN
		Select @strsql=''   
		Select @strsql= 'insert into #RunTime1(MachineID,machineinterface,Component,Compinterface,Operation,Opninterface,
		...
		Case when autodata.msttime< T.fromtm then T.fromtm else autodata.msttime end,   
		Case when autodata.ndtime> T.totm then T.totm else autodata.ndtime end,     
		T.fromtm,T.totm,0,autodata.id,componentoperationpricing.Cycletime,componentoperationpricing.SubOperations FROM #T_autodata autodata  with(nolock)
		INNER JOIN  machineinformation ON autodata.mc = machineinformation.InterfaceID   
		INNER JOIN componentinformation ON autodata.comp = componentinformation.InterfaceID    
		...
		cross join(select distinct fromtm,totm from #DailyProductionFromAutodataT2) as T
		where
		((autodata.msttime >= T.fromtm AND autodata.ndtime <=t.totm)
		OR ( autodata.msttime < T.fromtm  AND autodata.ndtime <= t.totm AND autodata.ndtime > T.fromtm )
		OR ( autodata.msttime >= T.fromtm   AND autodata.msttime < t.totm AND autodata.ndtime >  t.totm )
		OR ( autodata.msttime < T.fromtm   AND autodata.ndtime >  t.totm) ) '
		select @strsql = @strsql 
		select @strsql = @strsql + ' order by autodata.msttime'  
		print @strsql  
		exec (@strsql)

		--select * from #RunTime where FromTm>='2021-11-10 06:00:00.000' and ToTm<='2021-11-10 14:00:00.000' order by autodataid asc
		--return


		insert into #FinalRunTime1(MachineID,Component,operation,machineinterface,Compinterface,Opninterface,batchid,BatchStart,BatchEnd,FromTm,ToTm,stdtime,Actual,Target,Runtime,SubOperations) 
		...
		--select * from #FinalRunTime where FromTm>='2021-11-10 14:00:00.000' and ToTm<='2021-11-10 22:00:00.000'
		--return


		update #FinalRunTime1 set Runtime=datediff(s,BatchStart,BatchEnd)


		If (SELECT ValueInText From CockpitDefaults Where Parameter ='Ignore_Ptime_4m_PLD')<>'N'
		BEGIN

			Update #FinalRunTime1 set Runtime=Isnull(Runtime,0) - Isnull(T3.pdt,0) 
			from (
			...
			) T3 inner join #FinalRunTime1 T on T.machineinterface=T3.machineinterface and T.BatchStart=T3.BatchStart and  T.BatchEnd=T3.BatchEnd and T.Fromtm=T3.Fromtm and T.ToTm=T3.ToTm

		ENd

		update #FinalRunTime1 set target=(t.tcount)
		from
		(
		...
		inner join #FinalRunTime1 F on co.machineid=F.machineid and
		CO.Componentid=F.Component and Co.operationno=F.Operation where co.cycletime<>0
		)t INNER JOIN #FinalRunTime1 F ON F.MachineID=T.MachineID AND F.Component=T.component AND F.Operation=T.operation AND  F.BatchStart=T.BatchStart AND F.BatchEnd=T.BatchEnd


		UPDATE #DailyProductionFromAutodataT2 SET Targetcount=ISNULL(t1.tgt,0)
		FROM
		(SELECT MachineID,MachineInterface,component,Compinterface,Operation,fromtm,totm, SUM(ISNULL(Target,0)) AS tgt FROM #FinalRunTime1
		GROUP BY MachineID,MachineInterface,Component,Compinterface,Operation,FromTm,ToTm
		)t1 INNER JOIN #DailyProductionFromAutodataT2 ON #DailyProductionFromAutodataT2.MachineID = t1.MachineID 
		and #DailyProductionFromAutodataT2.Component = t1.Component 
		 and #DailyProductionFromAutodataT2.Operation = t1.Operation and #DailyProductionFromAutodataT2.FromTm=t1.FromTm and #DailyProductionFromAutodataT2.ToTm=t1.ToTm
...
and T2.Cdate=T1.Cdate and isnull(T1.GroupID,'')=isnull(T2.GroupID,'') and T1.Component=T2.Component and T1.Operation=T2.Operation
...
--		T1.Pdate=T2.Cdate 
--		and isnull(T1.GroupID,'')=isnull(T2.GroupID,'')
...
---------------------Removed and added below (handled duplicated sum of rej count)----------------------------------

--UPDATE #DailyProductionFromAutodataT1 SET QualityEfficiency= ISNULL(QualityEfficiency,0) + IsNull(T1.QE,1) 
--FROM(Select T1.MachineID,T1.FromTime,cast((Sum(isnull(CountShiftTotal,0))-Sum(IsNull(RejCount,0))) as float)/CAST((Sum(IsNull(CountShiftTotal,0))) AS Float)As QE
--From #DailyProductionFromAutodataT1 T1
--INNER JOIN #DailyProductionFromAutodataT2  T2 ON
--		T1.MachineID = T2.MachineID and
--		T1.Pdate=T2.Cdate 
...
UPDATE #DailyProductionFromAutodataT1 SET QualityEfficiency= ISNULL(QualityEfficiency,0) + IsNull(T1.QE,1) 
FROM(Select T1.MachineID,T1.FromTime,cast((Sum(isnull(CountShiftTotal,0))-isnull(Sum(distinct RejCount ),0)) as float)/CAST((Sum(IsNull(CountShiftTotal,0))) AS Float)As QE
From #DailyProductionFromAutodataT1 T1
INNER JOIN #DailyProductionFromAutodataT2  T2 ON
		T1.MachineID = T2.MachineID and
		T1.Pdate=T2.Cdate 
...
select  #DailyProductionFromAutodataT1.MachineID,
	...
	isnull(#DailyProductionFromAutodataT2.Component,'') as Component,
		isnull(#DailyProductionFromAutodataT2.CompDescription,'') as CompDescription,
	isnull(#DailyProductionFromAutodataT2.Operation,'') as Operation,
		isnull(#DailyProductionFromAutodataT2.OpnDescription,'') as OpnDescription,
	...
	isnull(#DailyProductionFromAutodataT2.TargetCount,0) as Target,
	isnull(Round(#DailyProductionFromAutodataT2.CountShiftTotal,2),0) as CountShiftTotal,
	isnull(round((isnull(#DailyProductionFromAutodataT2.TargetCount,0)-isnull(#DailyProductionFromAutodataT2.CountShiftTotal,0)),2),0) as Diff
...
LEFT OUTER JOIN #DailyProductionFromAutodataT2 ON
	...
	and isnull(#DailyProductionFromAutodataT1.GroupID,'') = isnull(#DailyProductionFromAutodataT2.GroupID,'')
	...
	order by #DailyProductionFromAutodataT1.GroupID,#DailyProductionFromAutodataT1.Pdate,#DailyProductionFromAutodataT1.MachineID
END
...
Begin
	...
	from 	
	(select distinct #DailyProductionFromAutodataT1.Pdate,#DailyProductionFromAutodataT1.MachineID,
	#DailyProductionFromAutodataT1.GroupID,
	ISNULL(ROUND(#DailyProductionFromAutodataT1.ProductionEfficiency,2),0) as ProductionEfficiency,
	ISNULL(ROUND(#DailyProductionFromAutodataT1.AvailabilityEfficiency,2),0) as AvailabilityEfficiency,
	isnull(Round(#DailyProductionFromAutodataT1.QualityEfficiency,2),0) as QualityEfficiency,
	ISNULL(ROUND(#DailyProductionFromAutodataT1.OverallEfficiency,2),0) as OverallEfficiency,
	isnull(#DailyProductionFromAutodataT1.DownTime,0) as frmtDownTime from   #DailyProductionFromAutodataT1 
inner join machineinformation M on #DailyProductionFromAutodataT1.MachineID=M.machineid
LEFT OUTER JOIN #DailyProductionFromAutodataT2 ON
	#DailyProductionFromAutodataT1.MachineID = #DailyProductionFromAutodataT2.MachineID
	and isnull(#DailyProductionFromAutodataT1.GroupID,'') = isnull(#DailyProductionFromAutodataT2.GroupID,'')
